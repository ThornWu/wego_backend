...
def DoUpdateLabel():
    count = 0
    error_count = 0
    with open(os.path.join(PROJECT_ROOT,"Origin","Result","location_cluster_LA_min.csv"), 'r', encoding='ISO-8859-1') as f:
        f_csv = csv.reader(f)
        headers = next(f_csv) # 取出表头
        for item in f_csv:
            venueid = item[0]
    ...
    count = 0
    with open(os.path.join(PROJECT_ROOT,"Origin","Result","location_cluster_NYC_min.csv"), 'r', encoding='ISO-8859-1') as f:
        f_csv = csv.reader(f)
        headers = next(f_csv) # 取出表头
        for item in f_csv:
            venueid = item[0]
